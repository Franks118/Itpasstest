...
@Component({
      ...
      <div *ngIf="review() as data" class="review-container">
        <header class="review-header-meta">
          <div class="header-left">
            ...
            <h1>Answer Key: {{ data.exam.title }}</h1>
          </div>
          <div class="review-summary-pill pill info">
            {{ data.exam.questions.length }} Questions
          </div>
        </header>

        <div class="review-list">
          <article *ngFor="let q of data.exam.questions; let i = index" class="card review-card">
  ...
  styles: [`
    ...
    .review-summary-pill { font-weight: 700; background: var(--accent-soft); color: var(--accent-primary); }
...
export class AnswerKeyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (examId) {
      this.loadAnswerKey(examId);
    } else {
      this.error.set('Invalid exam reference.');
    }
  }

  loadAnswerKey(examId: number): void {
    this.api.getAnswerKey(examId).subscribe({
      next: (data) => this.review.set(data),
      error: (err) => this.error.set(err.error?.message || 'Failed to load answer key.')
    });
  }
}
